...
### 3.4 日记模块
- **列表页**：搜索栏 + 日记卡片流（日期/天气/标题/预览/心情），点击查看详情，左滑露出卡片右侧“修改/删除”按钮
- **写日记页**：元数据标签 + 标题 + 内容 + 底部工具栏；录音低于1秒自动判定无效并提示重录；历史缺失语音文件会提示重新录音
...
## 2026-02-20 修复补充（语音与交互）
- 日记详情弹窗新增“删除”按钮，进入详情后可直接编辑/删除。
- 日记语音标记解析升级为大小写不敏感并兼容旧路径格式。
- 语音播放增加兼容数据源与错误提示，避免点击后无响应。
- 语音文件名升级为毫秒级，减少同秒录音冲突导致的历史语音丢失风险。

## 2026-02-20 日记交互补充
- 日记列表已切换为左滑触发“编辑/删除”。
- 卡片底部常驻编辑/删除按钮已移除，避免误触与视觉拥挤。
- 长按不再触发编辑/删除，交互统一为左滑露出按钮后点击。
- 左滑加入开合过渡动画与右滑收起（轻拉即可），优化操作丝滑度；操作区样式简化为直出按钮，减少圆角边缘卡顿感。
